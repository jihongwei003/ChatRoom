...
class SingleServer implements Runnable {
	...
	public void run() {
		...
		try {
			...
			/** 将当前连接存入socket连接表、文件socket表、声音socket表中 */
			...
			System.out
					.println("【剩余在线用户：" + users.getOnlineUsers().size() + "】");
			System.out.println("【剩余连接数：" + users.getSocketsMap().size() + "】");
			System.out.println("【剩余文件客户端连接数：" + users.getFileSocketsMap().size()
					+ "】");
			System.out.println("【剩余声音客户端连接数："
